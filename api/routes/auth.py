...
def validate_token(token: str):
    ...
    try:
        payload = jwt.decode(
            token, user_db["secret_key"], algorithms=["HS256"])
...
# Function to generate a JWT token for a given username and password
def generate_token(username: str, password: str):
    ...
    access_token_expires = timedelta(
        minutes=user_db["access_token_expire_minutes"])
    access_token_payload = {"sub": username,
                            "exp": datetime.utcnow() + access_token_expires}
    access_token = jwt.encode(access_token_payload,
                              user_db["secret_key"], algorithm="HS256")
    return access_token

# Dependency function to validate the JWT token
